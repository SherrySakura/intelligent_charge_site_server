...
window.onload = function(){
	...
	function check_regist(){
		...
		/*发送ajax请求*/
		// ajax('post','./register','name='+name+'&password='+password+'&phone='+phone+'&mail='+mail,function(data){
		// 	//服务器成功返回
		//
		// 	if(data==''){ //注册失败
		...
		// 	}
		// 	else{
		// 		showModalBox('注册成功，3秒后自动跳转到登录页面...');
		// 		var timer = setTimeout(gotoLoginPage,3000);		//执行一次
		// 	}
		//
		// });
		var register_json = {"name":name,"password":password,"phone":phone,"mail":mail};
		var register_json_str = JSON.stringify(register_json);
		// alert(register_json_str);
		$.ajax({
			url:"/registerServlet",
			type:"post",
			...
			success:function(data){
				...
				if (data.register == 'false') {
					showModalBox('注册失败，用户名或手机号或邮箱已被他人注册！');
				} else {
