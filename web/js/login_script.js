...
window.onload = function(){
    ...
    function check_login(){
		...
		//
		// });
		var login_json = {"name":name, "password":password};
		var login_json_str = JSON.stringify(login_json);
		$.ajax({
			url:"/csu/loginServlet",
			type:"post",
			data:{"loginJson":login_json_str},
			dataType:"json",
			success:function (data) {
				if (data.user != null){
					showModalBox('登录成功');
					open("index.html", "_self"); // 登陆成功后再次跳转到首页[注：做到这里就没做了]
					// 如果还要做的话，思路为
					// 因为已经登陆成功了跳转到了首页，所以每次加载首页应该要去请求一个Servlet判断登陆与否状态
					// 再通过session获取到User对象将[登陆|注册]换为[欢迎XXX]
				} else {
					showModalBox('登录失败，用户名或密码错误');
				}
			},
			error:function (data) {
				alert("系统异常");
